...
function createConsentController({ store, chain }) {
  return {
    grant: async (req, res) => {
      ...
      if (patient.wallet.toLowerCase() !== req.user.wallet.toLowerCase()) {
        return res.status(403).json({ error: "Only the patient can grant consent" });
      }

      const provider = await store.providerByWallet(req.body.wallet);
      ...
      try {
        ...
        res.status(502).json({ error: "Consent grant failed", details: e.message });
      }
    },
    ...
    revoke: async (req, res) => {
      ...
      try {
        ...
        res.status(502).json({ error: "Consent revoke failed", details: e.message });
      }
    },
