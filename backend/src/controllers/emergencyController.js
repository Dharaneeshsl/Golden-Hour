...
function createEmergencyController({ store, chain }) {
  const canAudit = (req, patient) =>
    req.user.role === "admin" || patient.wallet.toLowerCase() === req.user.wallet.toLowerCase();
  ...
  return {
    create: async (req, res) => {
      ...
      try {
        ...
        if (chain.enabled) await chain.triggerEmergency(chainPatientId, req.provider.wallet, justification.trim());

        const expiresAt = new Date(Date.now() + 15 * 60 * 1000).toISOString();
        const event = await store.addAudit({
          id: crypto.randomUUID(),
          patientId,
          actor: req.provider.wallet,
        ...
        // Never return critical clinical data from the creation endpoint.
        // The dedicated critical endpoint enforces the time-bound grant and actor check.
        res.json({ accessId: event.id, patientId, expiresAt, audit: event });
      } catch (e) {
        res.status(502).json({ error: "Emergency access failed", details: e.message });
      }
    },
    ...
    critical: async (req, res) => {
      ...
      if (!event || event.action !== "EMERGENCY_BREAK_GLASS") {
        return res.status(404).json({ error: "Emergency access not found" });
      }
      const expiresAt = new Date(event.expiresAt).getTime();
      if (!Number.isFinite(expiresAt) || expiresAt <= Date.now()) {
        return res.status(410).json({ error: "Emergency access expired" });
      }
      const patient = await store.patientById(event.patientId);
      if (!patient) return res.status(404).json({ error: "Patient not found" });
      if (req.user.role !== "admin" && event.actor.toLowerCase() !== req.user.wallet.toLowerCase()) {
        return res.status(403).json({ error: "Emergency access denied" });
      }
      res.json({ patientId: patient.id, critical: patient.critical, expiresAt: event.expiresAt });
    },

    audit: async (req, res) => {
      const patient = await store.patientById(req.params.patientId);
      if (!patient) return res.status(404).json({ error: "Patient not found" });
      if (!canAudit(req, patient)) return res.status(403).json({ error: "Audit access denied" });
