...
contract EmergencyAccess is Ownable {
    IRegistry public immutable registry; IProviderAccess public immutable accessControl; IAudit public immutable auditLog;
    event EmergencyTriggered(uint256 indexed patientId, address indexed doctor, string justification);
    constructor(address registryAddress, address accessAddress, address auditAddress, address initialOwner) Ownable(initialOwner) { registry=IRegistry(registryAddress); accessControl=IProviderAccess(accessAddress); auditLog=IAudit(auditAddress); }
    function triggerEmergencyAccess(uint256 patientId, string calldata justification) external { _trigger(patientId,msg.sender,justification); }
    function triggerEmergencyAccessFor(uint256 patientId, address doctor, string calldata justification) external onlyOwner { _trigger(patientId,doctor,justification); }
    function _trigger(uint256 patientId,address doctor,string calldata justification) internal {
        ...
        auditLog.logAccess(patientId, doctor, justification, "EMERGENCY_CRITICAL_ONLY");
        emit EmergencyTriggered(patientId, doctor, justification);
    }
}
