...
contract MedicalRecord is Ownable {
    ...
    constructor(address accessControlAddress, address initialOwner) Ownable(initialOwner) { accessControl = IGoldenHourAccess(accessControlAddress); }
    ...
    function addRecordFor(uint256 patientId, address doctor, string calldata recordType, string calldata ipfsHash, bytes32 encryptedHash) external onlyOwner {
    ...
    function _add(uint256 patientId, string calldata recordType, string calldata ipfsHash, bytes32 encryptedHash, address doctor) internal {
        uint256 version = _records[patientId].length + 1;
        _records[patientId].push(Record(version, recordType, ipfsHash, encryptedHash, doctor, uint64(block.timestamp)));
        emit RecordAdded(patientId, version, recordType, doctor);
    }

    function getRecords(uint256 patientId) external view returns (Record[] memory) { require(accessControl.checkAccess(patientId, msg.sender), "No patient access"); return _records[patientId]; }
